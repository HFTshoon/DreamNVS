...
from traj_enc_dec import TransformerAutoencoder
...
# 모델 초기화 및 학습 설정
model = TransformerAutoencoder(input_dim=7, hidden_dim=512, encoding_dim=512, num_heads=8, num_layers=6, max_len=max_len)
...
for epoch in range(epochs):
    ...
    for batch, (x, seq_len) in enumerate(dataloader):
        ...
        model_mask = torch.ones_like((x[:, :, 0]))
        for i in range(len(seq_len)):
            model_mask[i, :2*seq_len[i]] = 0
        ...
        # 모델로 입력 시퀀스를 복원
        output = model(x, max_len, model_mask)

        # make output of mask to zero
        mask = torch.ones_like(x)
        for i in range(len(seq_len)):
            mask[i, 2*seq_len[i]:] = 0
        mask = mask.to(device)
        output = output * mask
        ...
        # if loss_q_norm.item() > 0.1:
        #     loss = loss_q_norm + loss_t_l2
        # else:
    ...
    if epoch%10 == 0:
        ...
        with open(f"results_transformer/output_{epoch}.txt", "w") as f:
            f.write(txt_content)
