...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=True, focal_break=20, **kwargs):
        ...
        self.im_poses = nn.ParameterList(self.rand_pose(self.POSE_DIM) for _ in range(self.n_imgs))  # camera poses pose_dim=7,qx,qy,qz,qi,tx,ty,tz
        self.im_focal = nn.Parameter(torch.FloatTensor([self.focal_break * np.log(max(self.imshapes[0][0], self.imshapes[0][1]))]))
        # self.im_focals = nn.ParameterList(torch.FloatTensor(
        #     [self.focal_break*np.log(max(H, W))]) for H, W in self.imshapes)  # camera intrinsics, convert this to const intrinsics. 
    ...
    def get_focals(self):
        ...
        log_focals = torch.stack([self.im_focal] * len(self.imshapes), dim=0)
