...
#     abs_w2cs = [cam_param for cam_param in cam_params]
...
def get_relative_pose(cam_params):
    ...
    for pose in poses:
        ...
        relative_pose[:3, :3] = np.dot(start_pose[:3, :3].T, pose[:3, :3])
        relative_pose[:3, 3] = pose[:3, 3] - start_pose[:3, 3]
        relative_seq.append(relative_pose)
    return relative_seq

if __name__ == '__main__':
    device = "cuda"
    pose_adaptor_ckpt = "./checkpoints/CameraCtrl.ckpt"
    ...
    wrong_keys = ["encoder_down_attention_blocks.0.0.attention_blocks.0.pos_encoder.pe",
                "encoder_down_attention_blocks.0.1.attention_blocks.0.pos_encoder.pe",
                "encoder_down_attention_blocks.1.0.attention_blocks.0.pos_encoder.pe",
                "encoder_down_attention_blocks.1.1.attention_blocks.0.pos_encoder.pe",
                "encoder_down_attention_blocks.2.0.attention_blocks.0.pos_encoder.pe",
                "encoder_down_attention_blocks.2.1.attention_blocks.0.pos_encoder.pe",
                "encoder_down_attention_blocks.3.0.attention_blocks.0.pos_encoder.pe",
                "encoder_down_attention_blocks.3.1.attention_blocks.0.pos_encoder.pe"]

    for name in wrong_keys:
        param = pose_encoder_state_dict[name]
        param = param.unsqueeze(-1).unsqueeze(-1)
        pose_encoder_state_dict[name] = param
    ...
    with open("./demo/005/trajectory.json", "r") as f:
        cam_params = json.load(f)["trajectory"]
    cam_params = np.array(cam_params, dtype=np.float32)

    with open("./demo/005/intrinsics.json", "r") as f:
        intrinsic = json.load(f)

    focal = intrinsic["focals"][0]
    cx = intrinsic["principal_points"][0][0]
    cy = intrinsic["principal_points"][0][1]

    intrinsic = np.asarray([[focal,
                             focal,
                             cx,
                             cy]], dtype=np.float32)
                            # for _ in range(len(cam_params))], dtype=np.float32)
    ...
    print(c2ws.shape)
    plucker_embedding = ray_condition(K, c2ws, 256, 384, device='cpu')[0].permute(0, 3, 1, 2).contiguous()  # V, 6, H, W
    print(plucker_embedding.shape)
    plucker_embedding = plucker_embedding[None].to(device)  # B V 6 H W
    print(plucker_embedding.shape)
    pose_embedding = rearrange(plucker_embedding, "b f c h w -> b c f h w").to(dtype=torch.float32)
    print(pose_embedding.shape, pose_embedding.dtype)

    bs = pose_embedding.shape[0]
    assert pose_embedding.ndim == 5
    pose_embedding_features = pose_encoder(pose_embedding)       # bf, c, h, w
