...
class GuidanceModel(nn.Module):
    def __init__(
        ...
        self.encoder_3d = CrossAttentionEncoder(2*3*num_freqs_3d, nhead_3d, dim_feedforward_3d).to(device)
        self.encoder_traj = CrossAttentionEncoder(2*8*num_freqs_traj, nhead_traj, dim_feedforward_traj).to(device)
